...
export default async ({ adaptedClassesStructs, protocolsExcerpt }) => {
  const candidates = adaptedClassesStructs.filter(a => (a && a.protocols && a.protocols.length))
  if (!candidates || !candidates.length) {
    return
  }
  candidates.forEach(adaptedClassesStruct => {
    const { protocols, item: { className } } = adaptedClassesStruct
